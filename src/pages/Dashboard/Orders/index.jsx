...
import { Typography, Button, Table, Tag, Modal, Form, Select, Tooltip, Dropdown, Popconfirm } from 'antd'
...
const Orders = () => {
    ...
    // ── Fetch Orders ──────────────────────────────────────────────────
    const getDocuments = () => {
        axios.get(window.api + "/api/orders/all", {
            headers: { Authorization: `Bearer ${localStorage.getItem("jwt")}` },
        })
            .then(res => {
                const { status, data } = res
                if (status === 200) { setDocuments(data.orders) }
            })
            .catch(err => {
                console.log("err", err)
    ...
    const handleDelete = (order) => {
        ...
        axios.delete(window.api + "/api/orders/delete/" + order.id, {
            ...
            .catch(err => {
                ...
                const status = err.response ? err.response.status : 500
                const data = err.response ? err.response.data : { message: "Internal server error" }
                if (status === 401 || status === 403) { window.toastify(data.message, "error") }
                else { window.toastify("Something went wrong while deleting the order", "error") }
            })
    ...
    // ── Update Order Status ───────────────────────────────────────────
    const handleUpdate = () => {
        const { orderStatus } = form.getFieldsValue()

        if (!orderStatus) { return window.toastify("Please select an order status", "error") }
        ...
        axios.patch(
            ...
            .then(res => {
                ...
                if (status === 200) {
                    ...
                    setDocuments(prev => prev.map(doc =>
                        doc.id === orderToEdit.id ? data.order : doc
                    ))
                    resetModal()
                }
            })
            .catch(err => {
                ...
                const status = err.response ? err.response.status : 500
                const data = err.response ? err.response.data : { message: "Internal server error" }
                if (status === 400 || status === 403) { window.toastify(data.message, "error") }
                else { window.toastify("Something went wrong", "error") }
            })
        ...
        axios.patch(
            ...
            .then(res => {
                ...
                if (status === 200) {
                    ...
                    setDocuments(prev => prev.map(doc => doc.id === order.id ? data.order : doc))
                }
            })
            .catch(err => {
                console.error(err)
    ...
    const columns = [
                    ...
                    {products.map((p, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <img
                                src={p.imageURL}
                                alt={p.name}
                                style={{ width: 36, height: 36, objectFit: 'cover', borderRadius: 6, border: '1px solid #eee', flexShrink: 0 }}
                            />
        ...
        },
        {
            ...
            render: (addr) => (
                <div style={{ fontSize: 13, lineHeight: 1.6 }}>
                    ...
                    <div style={{ color: '#888' }}>{addr.address}, {addr.city}</div>
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            key: "orderStatus",
            render: (status) => (
                <Tag color={orderStatusColors[status] || 'default'} style={{ textTransform: 'capitalize', fontWeight: 600 }}>
                    {status}
                </Tag>
            ),
        },
        ...
        },
        {
            ...
            key: "paymentMethod",
            render: (val) => (
                <Tag color="purple" style={{ textTransform: 'uppercase' }}>{val}</Tag>
            ),
        },
        {
            title: "Date",
            dataIndex: "createdAt",
            key: "createdAt",
            render: (val) => new Date(val).toLocaleDateString('en-PK', {
...

        {
            title: "Action", key: 'action', render: (_, record) => {
                ...
                return (
                    <Dropdown menu={{
                        items: [
                            { label: "Mark as Shipped", key: "shipped", onClick: () => handleMarkShipped(record), disabled: record.orderStatus === "shipped" || record.orderStatus === "delivered" },
                            { label: <Popconfirm title="Are you sure?" onConfirm={() => handleDelete(record)}>Delete</Popconfirm>, key: "delete", danger: true }
                        ]
                    }}>
            ...
            >
                {orderToEdit && (
                    <div style={{ marginBottom: 16, padding: '12px 16px', background: '#f8f9fc', borderRadius: 10, border: '1px solid #e5e7eb' }}>
                        ...
                        <p style={{ margin: 0, fontWeight: 700, color: '#1d3557' }}>Rs. {Number(orderToEdit.totalAmount).toLocaleString()}</p>
                    </div>
                )}

                <Form form={form} layout="vertical" className='mt-3'>
                    <Form.Item label="Order Status" name="orderStatus" required>
                        <Select
